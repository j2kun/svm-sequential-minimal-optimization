...
function createProjectionSVG(v, h) {
  ...
  var p = { vector: v, hyperplane: h }; // need both to compute the projection
  var projected = svg.append("line").datum(p);
  var projectedArrowhead = svg.append('g').datum(p).append('path');
...
function chooseProjectionColor(d) {
  var signedLength = (0, _geometry.innerProduct)(d.vector, d.hyperplane);
  return signedLength >= 0 ? 'green' : 'red';
}

function setupProjectionStyle(projectionSVG) {
  var vector = projectionSVG.vector,
      vectorArrowhead = projectionSVG.vectorArrowhead,
      projected = projectionSVG.projected,
      projectedArrowhead = projectionSVG.projectedArrowhead;
  ...
  projected.attr("x1", function (d) {
    return fromCartesianX(0);
  }).attr("y1", function (d) {
    return fromCartesianY(0);
  }).attr("id", "projected").attr("stroke-width", vectorStroke).attr("stroke", chooseProjectionColor);
  var triangleSymbol = d3.symbol().size(arrowheadSize).type(d3.symbolTriangle);
...
function setupBehavior(hyperplaneSVG, projectionSVG) {
  ...
  function setSpanningPosition(svg) {
    svg.attr("x1", function (d) {
      return fromCartesianX(d.spanningX1(width, height));
    }).attr("x2", function (d) {
      return fromCartesianX(d.spanningX2(width, height));
    }).attr("y1", function (d) {
      return fromCartesianY(d.spanningY1(width, height));
    }).attr("y2", function (d) {
      return fromCartesianY(d.spanningY2(width, height));
    });
  }
  ...
  function setProjectedArrowheadPosition(svg) {
    svg.attr('transform', function (d) {
      var vector = d.vector.project(d.hyperplane);
      var offset = vector.arrowheadOffset();
      var displayX = fromCartesianX(vector.x) + offset[0];
      var displayY = fromCartesianY(vector.y) + offset[1];
      var rotationFromVertical = offset[2];
      return "translate(" + displayX + " " + displayY + ") " + "rotate(" + rotationFromVertical + ")";
